    ...
    def parse_one_event(event):
        ...
        try:
            ...
            if event['dates']['start']['dateTime']:
                start = parser.parse(event['dates']['start']['dateTime'])
                new_event.start_time = (start - start.utcoffset())
            else:
                return None
            new_event.duration = datetime.timedelta(hours=3)
            new_event.end_time = new_event.start_time + new_event.duration

            category = event['classifications'][0]['segment']['name'].lower()
            ...
            new_event.cost = event['priceRanges'][0]['min'] if 'priceRanges' in event else 0
